...
const Billing = () => {
  ...
  // making the object for invoice to database saving
  ...
  const calculateSubTotal = (invoicedata) => {
    ...
    invoiceData.items.length > 0 &&
      invoicedata.items.map((item) => {
        const total = parseInt(item?.Addquantity) * parseInt(item?.unitPrice)
  ...
  const handleSave = () => {
    ...
    fetch('http://localhost:8080/api/Billing/', {
      ...
      body: JSON.stringify(bill), // Use the updatedAddData
    })
      .then((response) => response.json())
      .then((data) => {
  ...
  return (
    <>
                  ...
                  <CCardBody>
                    <div className="d-flex">
                      <CDropdown>
                        ...
                        <CDropdownMenu>
                          <CDropdownItem onClick={() => setSelectedCategory(null)}>
                            All
                          </CDropdownItem>
                          {sortedCategories &&
                          ...
                          <CTableRow key={index}>
                            ...
                            <CTableDataCell>
                              ...
                              <CFormInput
                                ...
                                onChange={(e) => handleChange(index, e)}
                                value={
                                  itemField?.Addquantity * itemField?.unitPrice -
                                  (itemField?.Addquantity *
                                    itemField?.unitPrice *
                                    itemField?.discount) /
                                    100
                                }
              ...
              <div>
                ...
                <CDropdown>
                  ...
                  <CDropdownMenu>
                    ...
                    <CDropdownItem onClick={() => handleCurrencyClick('Euros')}>
                      Euros
                    </CDropdownItem>
      ...
      <ReactToPdf targetRef={invoiceRef} filename="invoice.pdf" x={0.5} y={0.8} scale={0.63}>
        {({ toPdf }) => (
          <div style={{ textAlign: 'center' }}>
            <button className="btn btn-primary" style={{ margin: '30px' }} onClick={handleSave}>
              Save and Continue
            </button>
            <button className="btn btn-primary" onClick={toPdf} style={{ margin: '30px' }}>
              Invoice PDF
            </button>
          </div>
        )}
